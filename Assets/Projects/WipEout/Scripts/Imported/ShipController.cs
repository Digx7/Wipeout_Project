...
{
    /* Description --
     *  this script appears to be the main script that controlls the ship
     */
    /* Notes --
     *  this might have to be the most modified script in order update this project
     */

    public ShipSettings handling;
    ...
    //inputs
    public WipeoutInputs inputs;//input action map
    [SerializeField] private float horz, vert, accel, drift, stunt;// og inputs
    [SerializeField] private Vector2 lStick;// vector 2 for inputs
    ...
    void Start()
    {
        // sets up Rigidbody and HUD
        rb = GetComponent<Rigidbody>();
        HUD = GetComponent<ShipHUD>();

        // sets up ship and gets stats
    ...
    void GetInputs()
    {
        ...
        inputs.Ship.Accelerate.performed += ctx => accel = ctx.ReadValue<float>();
        inputs.Ship.Drift.performed += ctx => drift = ctx.ReadValue<float>();
        inputs.Ship.Steering.performed += ctx => lStick = ctx.ReadValue<Vector2>();
        inputs.Ship.Stunt.performed += ctx => stunt = ctx.ReadValue<float>();
    }
